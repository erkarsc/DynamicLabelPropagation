...
{
    return x.iter().fold(0.,|sum,x|sum + x).sqrt()
}
...
pub fn affinityMatrix(x:&Vec<Vec<f64>>, sigma:f64)->Vec<Vec<f64>>
{
    ...
    for i in 0..n
    {
        for j in 0..n
        {
            ...
            val = norm(&dif).powf(2.)/sigma; //find norm of vector
            w[i][j] = (-val).exp();
        }
    }
    return w
...
pub fn kNN_graph(mat:&Vec<Vec<f64>>,k:usize) -> Vec<Vec<f64>>
{
    let n = mat.len();
    let m = mat[0].len();
    for i in 0..
...
fn main()
{
    ...
    let _trainLabels = USPSlabels(&file1).unwrap();
    let _trainFeatures = USPSfeatures(&file2).unwrap();
